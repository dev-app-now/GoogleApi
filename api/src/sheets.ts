export async function createSheet(accessToken: string, title: string) {
	...
	await fetch(`https://www.googleapis.com/drive/v3/files/${sheet.spreadsheetId}/permissions`, {
		method: 'POST',
		headers: {
			Authorization: `Bearer ${accessToken}`,
			'Content-Type': 'application/json',
...
export async function updateRange(accessToken: string, spreadsheetId: string, range: string, values: any[][]) {
...
export async function updateStyle(accessToken: string, spreadsheetId: string, range: string, style: any) {
